...
import { Category, Product, ProductData, Images, Images64Bits } from './app.models';
...
@Injectable()
export class AppServiceProduct {
    ...
    constructor(public http:HttpClient, public snackBar: MatSnackBar, private _sanitizer: DomSanitizer) { 
    }
    ...
    //este necesito
    public getProducts(type): Observable<ProductData[]>{ 
        return this.http.get<ProductData[]>(this.url + type + '-products.json');
    }
    
    public convertProductDatblaArrayToProductArray(productDataArray: ProductData[]): Product[]{
        this.productArray=[]
        productDataArray.forEach(element => {
            element.product.images = this.converteImage(element.product.images)
            this.productArray.push(element.product);
        });

        return this.productArray;
    }

    public convertProductDataToProduct(productData):Product{
        productData.product.images= this.converteImage(productData.product.images)
        return productData.product;
    }

    public getProductById(id): Observable<ProductData>{
        return this.http.get<ProductData>(this.url + 'product-' + id + '.json');
    }
